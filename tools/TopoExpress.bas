...
Public Sub ExportarTopoExpress()
    ...
    Dim slotShapes As New Collection, slotTypes As New Collection
    Dim i As Long
    ...
    For i = 1 To pg.Shapes.Count
        ...
        If TipoSlot(shp) <> "" Then GoTo Proximo

        Dim internos As New Collection
        ColetarShapesSlotDentro shp, internos
        OcultarShapes internos
...
Proximo:
    ...
    If CriarZipPowerShell(pasta, zipPath) Then
        MsgBox "Pacote criado com sucesso!" & vbCrLf & vbCrLf & "ZIP: " & zipPath & vbCrLf & "Slots detectados: " & slotShapes.Count, vbInformation, "Topo Express"
    Else
        MsgBox "A pasta foi criada, mas o ZIP automático falhou." & vbCrLf & pasta, vbExclamation, "Topo Express"
    End If
    Exit Sub
...
Private Function TipoSlot(ByVal shp As Shape) As String
    ...
    If shp.Type = cdrTextShape Then
        Dim conteudo As String: conteudo = NormalizarToken(shp.Text.Story.Text)
...
Private Function NormalizarToken(ByVal s As String) As String
    s = Replace$(s, vbCrLf, "")
    s = Replace$(s, vbCr, "")
    s = Replace$(s, vbLf, "")
    s = Replace$(s, vbTab, "")
    NormalizarToken = UCase$(Trim$(s))
End Function

Private Sub ColetarSlotsRecursivo(ByVal shp As Shape, ByRef shapes As Collection, ByRef types As Collection)
    On Error Resume Next
    Dim typ As String: typ = TipoSlot(shp)
    If typ <> "" Then shapes.Add shp: types.Add typ: Exit Sub
...
Private Sub ColetarShapesSlotDentro(ByVal shp As Shape, ByRef result As Collection)
    On Error Resume Next
    ...
    For i = 1 To shp.Shapes.Count
        ...
        ElseIf child.Type = cdrGroupShape Then
            ColetarShapesSlotDentro child, result
        End If
    Next i
End Sub

Private Sub OcultarShapes(ByVal shapes As Collection)
    On Error Resume Next
    Dim i As Long
    For i = 1 To shapes.Count: shapes(i).Visible = False: Next i
End Sub

Private Sub MostrarShapes(ByVal shapes As Collection)
    On Error Resume Next
    Dim i As Long
    For i = 1 To shapes.Count: shapes(i).Visible = True: Next i
End Sub
...
Private Function TextoSlotJson(ByVal shp As Shape, ByVal typ As String, ByVal ph As Double) As String
    ...
    On Error Resume Next
    If shp.Type = cdrTextShape Then fonte = shp.Text.Story.Font: tamanho = shp.Text.Story.Size
    On Error GoTo 0
...
Private Function SanitizarNomeArquivo(ByVal s As String) As String
    ...
    s = Replace$(Trim$(s), " ", "_"): If s = "" Then s = "tema"
