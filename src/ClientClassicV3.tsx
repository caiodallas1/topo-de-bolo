...
import { Category, DEFAULT_CATEGORIES, DEFAULT_THEMES, TextSlot, Theme, TopperElement, TopperOrder } from './data/catalog';
...
function A4Editor({ elements, setElements, slots, setSlots, childName, age, selected, setSelected, interactive = false }: {
  ...
  setElements?: (v: TopperElement[]) => void;
  slots: TextSlot[];
  setSlots?: (v: TextSlot[]) => void;
  childName: string;
  age: string;
  selected: SelectedObject;
  setSelected?: (v: SelectedObject) => void;
  interactive?: boolean;
}) {
  ...
  return <svg ref={svgRef} viewBox="0 0 210 297" className="aspect-[210/297] w-full touch-none bg-white shadow-[0_12px_35px_rgba(0,0,0,.15)] ring-1 ring-zinc-200"
    ...
    {elements.map((element) => {
      const h = element.widthMm / Math.max(.01, element.aspect || 1);
      const active = selected?.kind === 'element' && selected.id === element.id;
      return <g key={element.id}>
        <image href={element.src} x={element.xMm} y={element.yMm} width={element.widthMm} height={h} preserveAspectRatio="xMidYMid meet"
        ...
        {active && interactive && <rect x={element.xMm} y={element.yMm} width={element.widthMm} height={h} fill="none" stroke="#ef4444" strokeWidth="1" strokeDasharray="3 2" pointerEvents="none" />}
      </g>;
    })}
    {slots.map((slot) => {
      ...
      const active = selected?.kind === 'text' && selected.type === slot.type;
      return <g key={slot.type}>
        <text x={slot.xMm} y={slot.yMm} dominantBaseline="hanging" fontFamily={slot.fontFamily || 'Arial'} fontSize={sizeMm} fill={slot.fill || '#111111'} stroke={slot.stroke || 'none'} strokeWidth={slot.strokeWidthMm || 0} paintOrder="stroke fill"
        ...
        {active && interactive && <rect x={slot.xMm - 2} y={slot.yMm - 2} width={Math.max(25, slot.widthMm)} height={Math.max(10, slot.heightMm)} fill="none" stroke="#2563eb" strokeWidth=".8" strokeDasharray="2 2" pointerEvents="none" />}
      </g>;
    })}
  </svg>;
}

export default function ClientClassicV3() {
  ...
  const filteredThemes = useMemo(() => {
    ...
    return themes.filter((t) => (categoryFilter === 'all' || t.categoryId === categoryFilter) && (!term || t.name.toLowerCase().includes(term) || t.description?.toLowerCase().includes(term)));
  ...
  const nameSlot = slots.find((s) => s.type === 'name');
  const ageSlot = slots.find((s) => s.type === 'age');
  const nameFontAvailable = !nameSlot?.fontFamily || installedFonts.some((font) => font.family.toLowerCase() === nameSlot.fontFamily?.toLowerCase());

  const selectTheme = (theme: Theme) => {
    setSelectedTheme(theme);
    setElements(theme.elements.map((e) => ({ ...e })));
    const nextSlots = theme.textSlots?.map((slot) => ({ ...slot })) || [];
    setSlots(nextSlots);
    setSelected(theme.elements[0] ? { kind: 'element', id: theme.elements[0].id } : (nextSlots[0] ? { kind: 'text', type: nextSlots[0].type } : null));
  };
  ...
  const finish = () => {
    ...
    const category = categories.find((c) => c.id === selectedTheme.categoryId);
    const created: TopperOrder = {
      code: makeCode(), createdAt: new Date().toISOString(), themeId: selectedTheme.id, themeName: selectedTheme.name,
      categoryName: category?.name || 'Sem categoria', childName: childName.trim(), age,
      fontFamily: nameSlot?.fontFamily || 'Arial', elements, textSlots: slots,
    };
    const old = readLocal<TopperOrder[]>(ORDERS_KEY, []);
    localStorage.setItem(ORDERS_KEY, JSON.stringify([created, ...old]));
    setOrder(created);
  };

  const restart = () => {
    setStep(0); setSearch(''); setCategoryFilter('all'); setSelectedTheme(null); setChildName(''); setAge(''); setElements([]); setSlots([]); setSelected(null); setOrder(null);
  };
    ...
    <main className="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
      ...
      <section className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm md:p-8">
        {step === 0 && <div>
          ...
          <div className="mt-6 flex gap-3 overflow-x-auto pb-2"><button onClick={() => setCategoryFilter('all')} className={`min-w-fit rounded-2xl border-2 px-5 py-3 font-black ${categoryFilter === 'all' ? 'border-red-600 bg-red-600 text-white' : 'border-zinc-200 bg-white'}`}>Todos</button>{categories.map((c) => <button key={c.id} onClick={() => setCategoryFilter(c.id)} className={`flex min-w-fit items-center gap-2 rounded-2xl border-2 px-4 py-3 font-black ${categoryFilter === c.id ? 'border-red-600 bg-red-600 text-white' : 'border-zinc-200 bg-white'}`}>{c.image && <img src={c.image} className="h-8 w-8 rounded-lg object-cover"/>}{c.name}</button>)}</div>
          <label className="my-5 flex min-h-16 items-center gap-3 rounded-2xl border-2 border-zinc-200 bg-zinc-50 px-4"><Search size={24} className="text-zinc-500"/><input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-transparent text-xl font-semibold outline-none" placeholder="Buscar tema..."/></label>
        ...
        {step === 1 && <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-2 text-lg text-zinc-600">Digite os dados. Quando o tema tiver @NOME/@IDADE, eles serão substituídos automaticamente no A4.</p>

          <div className="mt-8"><label className="mb-2 block text-xl font-extrabold">Nome do aniversariante</label><input value={childName} onChange={(e) => setChildName(e.target.value.slice(0,24))} placeholder="Ex.: Maria Clara" className="min-h-16 w-full rounded-2xl border-2 border-zinc-200 px-5 text-xl font-bold outline-none focus:border-red-500"/>{!nameSlot && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">Este tema foi importado sem slot @NOME. Reimporte com o macro v3 para o nome entrar na posição do Corel.</p>}{nameSlot && !nameFontAvailable && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">A fonte “{nameSlot.fontFamily}” ainda não está cadastrada no sistema.</p>}</div>

          <div className="mt-7"><p className="mb-3 text-xl font-extrabold">Idade</p><div className="flex flex-wrap gap-2">{Array.from({length:10},(_,i)=>String(i+1)).map((v)=><button key={v} onClick={()=>setAge(v)} className={`h-16 w-16 rounded-2xl border-2 font-black ${age===v?'border-red-600 bg-red-600 text-white':'border-zinc-200 bg-white'}`}>{v}</button>)}<button onClick={()=>setAge('')} className={`min-h-16 rounded-2xl border-2 px-5 font-black ${age===''?'border-zinc-900 bg-zinc-900 text-white':'border-zinc-200 bg-white'}`}>Sem idade</button></div>{!ageSlot && <p className="mt-3 rounded-xl bg-zinc-50 p-3 text-sm font-bold text-zinc-600">Para a idade aparecer no A4, coloque @IDADE no Corel e reimporte com o macro v3.</p>}</div>
        </div>}

        {step === 2 && selectedTheme && <div>
          ...
          <p className="mt-2 text-lg text-zinc-600">As imagens vêm prontas do Corel. Nome e idade substituem os slots originais do tema.</p>
          <div className="mt-8 grid gap-7 lg:grid-cols-[minmax(0,520px)_1fr]">
            ...
            <div className="rounded-3xl bg-zinc-50 p-6"><h2 className="text-2xl font-black">Objeto selecionado</h2>
              {selectedElement && <div className="mt-5 space-y-5"><p className="text-xl font-black">{selectedElement.name}</p><div><div className="flex justify-between font-bold"><span>Largura</span><span>{(selectedElement.widthMm/10).toFixed(1)} cm</span></div><input type="range" min={selectedElement.minWidthMm||20} max={selectedElement.maxWidthMm||120} value={selectedElement.widthMm} onChange={(e)=>updateElement(selectedElement.id,{widthMm:Number(e.target.value)})} className="mt-2 w-full"/></div></div>}
              {selectedSlot && <div className="mt-5 space-y-5"><p className="text-xl font-black">{selectedSlot.type==='name'?'Nome':'Idade'}</p><div><div className="flex justify-between font-bold"><span>Tamanho</span><span>{Math.round(selectedSlot.fontSizePt||32)} pt</span></div><input type="range" min="10" max="120" value={selectedSlot.fontSizePt||32} onChange={(e)=>updateSlot(selectedSlot.type,{fontSizePt:Number(e.target.value)})} className="mt-2 w-full"/></div><label className="block font-bold">Cor<input type="color" value={selectedSlot.fill||'#111111'} onChange={(e)=>updateSlot(selectedSlot.type,{fill:e.target.value})} className="mt-2 h-12 w-full rounded-xl"/></label><div className="rounded-xl border border-zinc-200 bg-white p-3"><p className="text-xs font-black uppercase tracking-wider text-zinc-400">Fonte automática do Corel</p><p className="mt-1 text-lg font-black">{selectedSlot.fontFamily || 'Não informada'}</p></div><div className="grid grid-cols-2 gap-3 text-sm font-bold"><div className="rounded-xl bg-white p-3">X: {(selectedSlot.xMm/10).toFixed(1)} cm</div><div className="rounded-xl bg-white p-3">Y: {(selectedSlot.yMm/10).toFixed(1)} cm</div></div></div>}
              {!selectedElement && !selectedSlot && <p className="mt-5 text-zinc-500">Clique em uma imagem ou no texto.</p>}
              {!nameSlot && <p className="mt-6 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">Sem @NOME neste pacote. Use o macro v3 e reimporte o tema.</p>}
              <div className="mt-7 rounded-2xl border border-zinc-200 bg-white p-4 text-sm font-semibold text-zinc-600"><strong>A4 real:</strong> 21 × 29,7 cm.</div>
            </div>
          </div>
        </div>}
