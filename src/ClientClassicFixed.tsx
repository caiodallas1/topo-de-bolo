...
function corelSlots(theme: Theme): TextSlot[] {
  return theme.textSlots?.map((slot) => ({ ...slot })) || [];
}
...
export default function ClientClassicFixed() {
  ...
  const selectTheme = (theme: Theme) => {
    ...
    const nextSlots = corelSlots(theme);
    setSlots(nextSlots);
  ...
  const finish = () => {
    ...
    const created: TopperOrder = { code: makeCode(), createdAt: new Date().toISOString(), themeId: selectedTheme.id, themeName: selectedTheme.name, categoryName: category?.name || 'Sem categoria', childName: childName.trim(), age, fontFamily: nameSlot?.fontFamily || 'Arial', elements, textSlots: slots };
  ...
  return <div className="min-h-screen bg-[#f7f7f8] text-zinc-900">
    ...
    <main className="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
      ...
      <section className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-sm md:p-8">
        ...
        {step === 1 && <div className="mx-auto max-w-4xl"><p className="text-sm font-extrabold uppercase tracking-wider text-red-600">Passo 2 de 4</p><h1 className="mt-2 text-3xl font-black md:text-4xl">Agora, personalize o que veio do Corel</h1><p className="mt-2 text-lg text-zinc-600">O sistema só mostra campos que existem no arquivo do tema.</p>{nameSlot ? <div className="mt-8"><label className="mb-2 block text-xl font-extrabold">Nome do aniversariante</label><input value={childName} onChange={(e) => setChildName(e.target.value.slice(0,24))} placeholder="Ex.: Maria Clara" className="min-h-16 w-full rounded-2xl border-2 border-zinc-200 px-5 text-xl font-bold outline-none focus:border-red-500"/><p className="mt-2 text-sm font-semibold text-zinc-500">Esse valor substitui o @NOME que veio do Corel. Não é criado um segundo texto.</p>{!nameFontAvailable && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-900">A fonte “{nameSlot.fontFamily}” ainda não está na biblioteca. O administrador precisa apenas enviar o arquivo da fonte; o nome é detectado automaticamente.</p>}</div> : <p className="mt-8 rounded-2xl bg-zinc-50 p-5 font-bold text-zinc-600">Este tema não possui @NOME no Corel, então nenhum nome será criado automaticamente.</p>}{ageSlot && <div className="mt-7"><p className="mb-3 text-xl font-extrabold">Idade</p><div className="flex flex-wrap gap-2">{Array.from({length:10},(_,i)=>String(i+1)).map((v)=><button key={v} onClick={()=>setAge(v)} className={`h-16 w-16 rounded-2xl border-2 font-black ${age===v?'border-red-600 bg-red-600 text-white':'border-zinc-200 bg-white'}`}>{v}</button>)}<button onClick={()=>setAge('')} className={`min-h-16 rounded-2xl border-2 px-5 font-black ${age===''?'border-zinc-900 bg-zinc-900 text-white':'border-zinc-200 bg-white'}`}>Sem idade</button></div></div>}{nameSlot && <div className="mt-8 rounded-2xl bg-zinc-50 p-5"><p className="font-black">Estilo automático do Corel</p><p className="mt-1 text-2xl" style={{fontFamily:nameSlot.fontFamily}}>{childName || 'Maria Clara'}</p><p className="mt-2 text-sm font-semibold text-zinc-500">Fonte: {nameSlot.fontFamily || 'Não informada'} • {Math.round(nameSlot.fontSizePt || 32)} pt</p></div>}</div>}
        {step === 2 && selectedTheme && <div><p className="text-sm font-extrabold uppercase tracking-wider text-red-600">Passo 3 de 4</p><h1 className="mt-2 text-3xl font-black md:text-4xl">Ajuste seu topo no A4</h1><p className="mt-2 text-lg text-zinc-600">As imagens já vêm prontas do Corel, sem contorno automático. O texto é o slot original @NOME/@IDADE do tema.</p><div className="mt-8 grid gap-7 lg:grid-cols-[minmax(0,520px)_1fr]"><A4Editor elements={elements} setElements={setElements} slots={slots} setSlots={setSlots} childName={childName} age={age} selected={selected} setSelected={setSelected} interactive/><div className="rounded-3xl bg-zinc-50 p-6"><h2 className="text-2xl font-black">Objeto selecionado</h2>{selectedElement && <div className="mt-5 space-y-5"><p className="text-xl font-black">{selectedElement.name}</p><div><div className="flex justify-between font-bold"><span>Largura</span><span>{(selectedElement.widthMm/10).toFixed(1)} cm</span></div><input type="range" min={selectedElement.minWidthMm||20} max={selectedElement.maxWidthMm||120} value={selectedElement.widthMm} onChange={(e)=>updateElement(selectedElement.id,{widthMm:Number(e.target.value)})} className="mt-2 w-full"/></div></div>}{selectedSlot && <div className="mt-5 space-y-5"><p className="text-xl font-black">{selectedSlot.type==='name'?'Nome':'Idade'}</p><div><div className="flex justify-between font-bold"><span>Tamanho</span><span>{Math.round(selectedSlot.fontSizePt||32)} pt</span></div><input type="range" min="10" max="120" value={selectedSlot.fontSizePt||32} onChange={(e)=>updateSlot(selectedSlot.type,{fontSizePt:Number(e.target.value)})} className="mt-2 w-full"/></div><label className="block font-bold">Cor<input type="color" value={selectedSlot.fill||'#111111'} onChange={(e)=>updateSlot(selectedSlot.type,{fill:e.target.value})} className="mt-2 h-12 w-full rounded-xl"/></label><div className="rounded-xl border border-zinc-200 bg-white p-3"><p className="text-xs font-black uppercase tracking-wider text-zinc-400">Fonte do Corel</p><p className="mt-1 text-lg font-black">{selectedSlot.fontFamily || 'Não informada'}</p><p className="mt-1 text-xs font-semibold text-zinc-500">Aplicada automaticamente. O cliente não precisa escolher nem digitar fonte.</p></div><div className="grid grid-cols-2 gap-3 text-sm font-bold"><div className="rounded-xl bg-white p-3">X: {(selectedSlot.xMm/10).toFixed(1)} cm</div><div className="rounded-xl bg-white p-3">Y: {(selectedSlot.yMm/10).toFixed(1)} cm</div></div></div>}{!selectedElement && !selectedSlot && <p className="mt-5 text-zinc-500">Clique em uma imagem ou no texto.</p>}<div className="mt-7 rounded-2xl border border-zinc-200 bg-white p-4 text-sm font-semibold text-zinc-600"><strong>A4 real:</strong> 21 × 29,7 cm.</div></div></div></div>}
        {step === 3 && selectedTheme && <div><p className="text-sm font-extrabold uppercase tracking-wider text-red-600">Passo 4 de 4</p><h1 className="mt-2 text-3xl font-black md:text-4xl">Confira o A4 final</h1><div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,520px)_1fr]"><A4Editor elements={elements} slots={slots} childName={childName} age={age} selected={null}/><div className="rounded-3xl bg-zinc-50 p-6"><h2 className="text-2xl font-black">Resumo</h2><div className="mt-5 space-y-3 text-lg"><p><strong>Tema:</strong> {selectedTheme.name}</p>{nameSlot && <p><strong>Nome:</strong> {childName||'Sem nome'}</p>}{ageSlot && <p><strong>Idade:</strong> {age?`${age} anos`:'Sem idade'}</p>}<p><strong>Elementos:</strong> {elements.length}</p></div><p className="mt-6 rounded-2xl bg-emerald-50 p-4 font-semibold text-emerald-900">Ao aprovar, esse A4 fica salvo no pedido e aparece automaticamente no painel da produção.</p></div></div></div>}
